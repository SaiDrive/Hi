...
const UserImageThumbnail: React.FC<UserImageThumbnailProps> = ({ image, onRemove, userId }) => {
  const { content: imageUrl, isLoading } = useGcsContent(image.url, userId);

  return (
    <div className="relative group h-16 w-16">
      {isLoading && (
        <div className="h-full w-full bg-slate-700 rounded-md flex items-center justify-center">
          <Spinner className="w-6 h-6" />
        </div>
      )}
      {imageUrl && (
        <img src={imageUrl} alt={image.name} className="h-16 w-16 object-cover rounded-md" />
      )}
      <button
